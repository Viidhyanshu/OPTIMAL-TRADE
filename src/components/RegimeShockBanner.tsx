...
export const RegimeShockBanner: React.FC<RegimeShockBannerProps> = ({ result }) => {
  ...
  // Formatted string without double sign artifacts (+$-)
  const formattedAlpha = alphaDollars >= 0 
    ? `+$${alphaDollars.toLocaleString()}` 
    : `-$${Math.abs(alphaDollars).toLocaleString()}`;
  ...
  return (
    <div className="bg-[#12131a] border border-white/5 rounded-2xl p-5 shadow-2xl space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-white text-base flex items-center gap-2">
              ...
              <span className="text-xs px-2.5 py-0.5 rounded-full bg-white/10 border border-white/10 text-white font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
        <div className="bg-[#181924] p-4 rounded-xl border border-white/5 space-y-1.5">
        ...
        <div className="bg-[#181924] p-4 rounded-xl border border-white/5 space-y-1 flex flex-col justify-between">
          <span className="text-slate-300 font-semibold">Shock Mitigation Alpha</span>
          <div className={`text-2xl font-extrabold font-mono ${alphaDollars >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
            {formattedAlpha}
          </div>
